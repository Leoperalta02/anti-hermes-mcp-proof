...
class HermesACPAgent(acp.Agent):
    ...
    def _publish_fallback_channel_response(
        self,
        final_response: str,
        user_text: str,
        kwargs: dict,
        ...
        """
        Hardened Fallback Publish Guard:
        Ensures human-facing assistant responses are published into the originating Buzz channel
        if not already sent by a tool call during the turn.
        
        Authoritative Security Architecture:
        - Strict Fail-Closed / Deny-by-Default.
        - Cryptographically derives executing agent's Nostr pubkey from BUZZ_PRIVATE_KEY via secp256k1.
        - Validates executing pubkey against external auto_publish_allowlist.json.
        - Zero text/name matching or prompt inspection for authorization.
        - Deduplication requiring successful delivery evidence.
        - Originating channel & thread resolution (structured first, regex fallback, never guess).
        """
        # 1. Feature Toggle Check
        auto_pub_flag = os.environ.get("BUZZ_ACP_AUTO_PUBLISH", "true").lower()
        if auto_pub_flag in ("false", "0", "no", "off"):
            logger.debug("[Publish Guard] Auto-publish disabled via BUZZ_ACP_AUTO_PUBLISH=false")
            return False

        if not final_response or not final_response.strip():
            return False

        clean_text = final_response.strip()
        if clean_text.startswith("Error:") or clean_text.startswith("[INTERRUPT_"):
            return False

        # 2. Derive Authoritative Executing Pubkey from Process In-Memory Private Key
        privkey_hex = os.environ.get("BUZZ_PRIVATE_KEY", "").strip()
        if not privkey_hex or len(privkey_hex) != 64:
            logger.debug("[Publish Guard] Denied: Invalid or missing BUZZ_PRIVATE_KEY in process.")
            return False
        ...
        try:
            ...
            priv_key_obj = ec.derive_private_key(priv_num, ec.SECP256K1(), default_backend())
            pub_numbers = priv_key_obj.public_key().public_numbers()
            executing_pubkey = pub_numbers.x.to_bytes(32, byteorder="big").hex().lower()
        except Exception as e:
            logger.warning("[Publish Guard] Pubkey derivation failed: %s", e)
            return False

        if not executing_pubkey:
            return False

        # 3. Load & Validate External Allowlist Config (Fail Closed / Deny by Default)
        config_paths = [
            r"C:\LEO-LAB-ANTIGRAVITY\hermes-state\auto_publish_allowlist.json",
            r"c:\LEO-LAB-ANTIGRAVITY\anti-hermes-mcp-proof\config\auto_publish_allowlist.json"
        ]
        allowlist_cfg = None
            ...
            if os.path.exists(cp):
                try:
                    with open(cp, "r", encoding="utf-8") as f:
                        allowlist_cfg = json.load(f)
                    break
                except Exception as e:
                    logger.warning("[Publish Guard] Error reading allowlist config %s: %s", cp, e)

        if not allowlist_cfg or not isinstance(allowlist_cfg, dict):
            logger.debug("[Publish Guard] Denied: Missing or unreadable allowlist config.")
            return False

        if not allowlist_cfg.get("auto_publish_enabled", False):
            logger.debug("[Publish Guard] Denied: auto_publish_enabled is not true in config.")
            return False

        allowed_pubkeys_raw = allowlist_cfg.get("allowed_pubkeys", [])
        if not isinstance(allowed_pubkeys_raw, list):
            logger.debug("[Publish Guard] Denied: allowed_pubkeys is not a list in config.")
            return False

        valid_allowed_pubkeys = set()
        for k in allowed_pubkeys_raw:
            if isinstance(k, str) and len(k.strip()) == 64 and re.match(r"^[0-9a-fA-F]{64}$", k.strip()):
                valid_allowed_pubkeys.add(k.strip().lower())

        if executing_pubkey not in valid_allowed_pubkeys:
            logger.debug("[Publish Guard] Denied: Pubkey %s is not in authorized allowlist.", executing_pubkey[:10])
            return False

        # 4. Deduplication: Check if a channel message was already successfully sent during this turn
        messages = result.get("messages", []) or []
        for msg in messages:
            if isinstance(msg, dict):
                msg_str = str(msg.get("content", ""))
                if ('"accepted": true' in msg_str or '"accepted":true' in msg_str) and ("event_id" in msg_str):
                    logger.info("[Publish Guard] Skipping fallback: Channel message was already successfully sent during turn.")
                    return False

        # 5. Resolve Originating Channel UUID & Thread ID
        channel_id = None
        thread_id = None

        meta = kwargs.get("_meta") or kwargs.get("metadata") or {}
        if isinstance(meta, dict):
            channel_id = meta.get("channel_id") or meta.get("channelId") or meta.get("channel")
            thread_id = meta.get("thread_id") or meta.get("threadId") or meta.get("reply_to")

        if not channel_id and user_text:
            chan_match = re.search(r"Channel:[^\n]*?\(#([a-f0-9\-]{36})\)", user_text)
            if not chan_match:
                chan_match = re.search(r"Channel:[^\n]*?#([a-f0-9\-]{36})", user_text)
            if not chan_match:
                chan_match = re.search(r"channel_id[:=]\s*([a-f0-9\-]{36})", user_text, re.IGNORECASE)
            if not chan_match:
                chan_match = re.search(r"#([a-f0-9\-]{36})", user_text)
            
            if chan_match:
                channel_id = chan_match.group(1)

        if not thread_id and user_text:
            thread_match = re.search(r"Thread:[^\n]*?\(#([a-f0-9\-]{36})\)", user_text)
            if not thread_match:
                thread_match = re.search(r"Thread:[^\n]*?#([a-f0-9\-]{36})", user_text)
            if not thread_match:
                thread_match = re.search(r"reply_to[:=]\s*([a-f0-9\-]{36})", user_text, re.IGNORECASE)
            if thread_match:
                thread_id = thread_match.group(1)

        if not channel_id:
            logger.debug("[Publish Guard] No destination channel UUID resolved. Preserving in Activity only.")
            return False

        buzz_exe = r"C:\Users\leope\AppData\Local\Buzz\buzz.exe"
        if not os.path.exists(buzz_exe):
            logger.warning("[Publish Guard] buzz.exe not found at %s", buzz_exe)
            return False

        relay_url = os.environ.get("BUZZ_RELAY_URL", "https://hiking-logan-drug-advocacy.trycloudflare.com").strip()
        ...
        try:
            ...
            if res.returncode == 0:
                try:
                    ...
                    event_id = resp_json.get("event_id", "unknown")
                except Exception:
                    event_id = "unknown"
                logger.info(
                    "[Publish Guard SUCCESS] ts=%s pubkey=%s channel=%s thread=%s event_id=%s response_len=%d",
                    ts, executing_pubkey[:10], channel_id, thread_id or "none", event_id, len(clean_text)
                )
                return True
            else:
                logger.error(
                    "[Publish Guard FAILED] ts=%s pubkey=%s channel=%s returncode=%d stderr=%s",
                    ts, executing_pubkey[:10], channel_id, res.returncode, res.stderr.strip()
                )
                return False
        except Exception as exc:
            logger.exception("[Publish Guard EXCEPTION] pubkey=%s channel=%s: %s", executing_pubkey[:10], channel_id, exc)
            return False
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # ---- FALLBACK PUBLISH GUARD ----
        if final_response and not interrupted and not suppress_interrupt_response:
            try:
                self._publish_fallback_channel_response(
                ...
                logger.error("[Publish Guard Error] %s", e, exc_info=True)
